...
class HeaderAuto
{
    ...
    public function callRecursive(array $array, bool $topLevel): string
    {
        ...
        $result = $topLevel ? "<$tagName class=TableOfContents>" : "<$tagName>";
...
ob_start(function (string $string): string {
    if (preg_match_all('/<h([1-6]) id=([^\s>]+|"[^"]+")>([^<]+)/', $string, $matches, PREG_SET_ORDER)) {

        // Initialize structure
        /** @noinspection HtmlUnknownAnchorTarget */
        $rootArray = new ReferenceArray(['<a href=#theTop>The LayerZip Specification</a>']);
        ...
        $finalArray = json_decode(json_encode($rootArray), true);
        $replace = new HeaderAuto($finalArray, 'ol');

        return str_replace('<!-- Favicond-render-TableOfContents -->', (string)$replace, $string);
    } else {
        return $string;
    }
});